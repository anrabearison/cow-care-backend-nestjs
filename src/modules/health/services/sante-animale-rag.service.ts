...
@Injectable()
export class SanteAnimaleRagService {
  private readonly logger = new Logger(SanteAnimaleRagService.name);
  ...
  async searchRelevantKnowledge(query: string, topK: number = 5): Promise<KnowledgeChunk[]> {
    ...
    try {
      ...
      const { data, error } = await this.supabase.rpc('rechercher_connaissances_sante', {
        query_embedding: embedding,
        nombre_resultats: topK,
  ...
  formatForPrompt(chunks: KnowledgeChunk[]): string {
    ...
    return chunks
      .map(chunk => `[${chunk.maladie} - ${chunk.section}]\n${chunk.contenu}\nSource: ${chunk.source}`)
      .join('\n\n---\n\n');
  }
}
