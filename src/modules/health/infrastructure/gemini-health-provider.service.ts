...
export class GeminiHealthProviderService implements HealthAiProvider {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    this.geminiModel = this.configService.get<string>('GEMINI_MODEL')?.trim() ?? 'gemini-1.5';
    this.geminiApiUrl = this.configService.get<string>('GEMINI_API_URL')?.trim() ?? 'https://generativelanguage.googleapis.com/v1/models';
